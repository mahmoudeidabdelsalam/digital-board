...
export default function HomePage() {
  ...
  const whatsappHref = 'https://wa.me/201001234567';
  ...
  return (
    <main className="relative overflow-x-hidden">
      <div className="fixed inset-x-0 top-0 z-50 h-1 bg-transparent">
        <div className="h-full rounded-full bg-gradient-to-r from-primary to-secondary" style={{ width: `${scrollProgress}%` }} />
      </div>
      ...
      <section id="hero" className="relative isolate overflow-hidden px-4 pb-24 pt-32 sm:px-6 lg:px-8">
        <div className="absolute inset-0 -z-10">
          <div className="absolute left-1/2 top-0 h-[420px] w-[420px] -translate-x-1/2 rounded-full bg-primary/30 blur-3xl" />
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1.1fr_0.9fr]">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-2xl">
            ...
            <h1 className="font-arabic text-4xl font-black leading-tight text-white sm:text-5xl lg:text-6xl">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <a href="#contact" className="inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-primary to-secondary px-6 py-3 font-semibold text-white shadow-glow transition-transform hover:scale-[1.02]">
                ابدأ مشروعك <ArrowLeft className="h-5 w-5" />
              </a>
              <a href={whatsappHref} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full border border-white/15 bg-white/8 px-6 py-3 font-semibold text-white backdrop-blur-xl transition-transform hover:scale-[1.02]">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.1 }} className="relative">
            <div className="absolute inset-0 rounded-[2rem] bg-gradient-to-br from-primary/25 to-secondary/25 blur-3xl" />
            <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/10 p-4 shadow-2xl backdrop-blur-2xl">
              <Image
      ...
      <section id="why-us" className="px-4 py-24 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-12 grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} className="rounded-[2rem] border border-white/10 bg-white/6 p-8 backdrop-blur-2xl">
              ...
              <div className="mt-8 space-y-4">
                {whyChooseUs.map((item) => (
                  <div key={item.title} className="flex items-start gap-3 rounded-2xl border border-white/10 bg-black/20 p-4">
                    <div className="mt-0.5 rounded-full bg-gradient-to-r from-primary to-secondary p-2">
      ...
      <section className="px-4 py-24 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl rounded-[2.5rem] border border-white/10 bg-gradient-to-r from-primary/30 to-secondary/30 p-8 shadow-glow backdrop-blur-2xl sm:p-12 lg:p-16">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-2xl">
              <h2 className="font-arabic text-3xl font-black text-white sm:text-4xl">
      ...
      <section id="contact" className="px-4 pb-24 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl rounded-[2rem] border border-white/10 bg-white/6 p-8 backdrop-blur-2xl sm:p-10">
          <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
            ...
            <div className="rounded-[1.5rem] border border-white/10 bg-black/25 p-6">
              <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
                ...
                <button type="submit" className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-primary to-secondary px-6 py-3 font-semibold text-white transition-transform hover:scale-[1.02]">
